package kabam.rotmg.lunarmarket.tabs {
...
import kabam.rotmg.lunarmarket.content.MemMarketBuyItem;

import kabam.rotmg.lunarmarket.content.MemMarketItem;

import kabam.rotmg.lunarmarket.signals.MemMarketBuySignal;
import kabam.rotmg.lunarmarket.signals.MemMarketSearchSignal;
import kabam.rotmg.lunarmarket.utils.DialogUtils;
import kabam.rotmg.lunarmarket.utils.ItemUtils;
import kabam.rotmg.lunarmarket.utils.SortUtils;
...
public class MemMarketBuyTab extends MemMarketTab
{
    ...
    public function MemMarketBuyTab(gameSprite:AGameSprite)
    {
        super(gameSprite);
    ...
    private function searchItemsFunc(first:Boolean = false) : void
    {
        ...
        /*if (first)
        {
            ...
            {
                if (ItemUtils.isBanned(ObjectLibrary.idToType_[w])) // Skip on banned items
                {
                    continue;
                }

                var preloaded:MemMarketItem = new MemMarketItem(this.gameSprite_, SEARCH_ITEM_SIZE, SEARCH_ITEM_SIZE, 80, ObjectLibrary.idToType_[w], null);
        ...
        { */
            for each (var i:String in ObjectLibrary.typeToDisplayId_)
            {
                if (i.indexOf(this.searchField_.text().toLowerCase()) >= 0)
                {
                    if (ItemUtils.isBanned(ObjectLibrary.idToType_[i])) /* Skip on banned items */
                    {
                        continue;
                    }
